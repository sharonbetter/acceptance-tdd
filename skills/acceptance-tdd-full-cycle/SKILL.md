...
description: >-
  验收驱动全链路：读懂需求；歧义/缺失/争议单列「待确认」，**用户视角发问**；**逐项请用户确认**，确认后**自动带出下一项**；ATDD 主表含**场景名称**与 **Given/When/Then**；**执行记录**写入与主稿同名的 **`.execution-log.md` 配套文件**（与主稿同目录；主稿仅一行链接）；**整体验收文档闸门**通过后再结构、TDD、**含 E2E 的外层测试**至全绿。
...
**先验收、后实现；验收文档成稿后先交用户确认，再动结构与产码；先单测/集成红绿，再跑通 E2E 等外层自动化；未全绿不宣称完成。**
