...
description: >-
  验收驱动全链路：需求澄清（只做必要确认，AI可自行判断的不阻塞）+ 任务规划（先列 TODO，逐项执行验证）+ ATDD 主表含**场景名称**与 **Given/When/Then**；**执行记录**写入与主稿同名的 **`.execution-log.md` 配套文件**（与主稿同目录；主稿仅一行链接）；**整体验收文档闸门**通过后再结构、TDD、**含 E2E 的外层测试**至全绿。
...
**先规划任务；先验收、后实现；验收文档成稿后先交用户确认，再动结构与产码；先单测/集成红绿，再跑通 E2E 等外层自动化；未全绿不宣称完成。**
...
拿到需求后，**先不深入细节**，先规划本轮完整任务清单：

1. **识别范围**：从 spec 中划出本轮要做的功能边界
2. **列出任务**：按顺序写出本轮所有任务（如 `[T1] 澄清阻塞项` / `[T2] 编写 ATDD 验收场景` / `[T3] 结构设计` / `[T4] TDD 实现` / `[T5] E2E 落地` / `[T6] 全绿验证`）
3. **AI 可自行判断的用假设标注，不阻塞**：凡 AI 有合理依据自行推断、且不影响用户核心利益的细节，**先用 `[假设]` 标注，填入验收文档**，不作为阻塞项；后续若发现假设错了再改
4. **只将真正影响实现、有多条可能路径的歧义/缺失列入待确认**：判断标准——若不澄清会导致实现方向错误，且 AI 无法自行推断，则列为阻塞项
5. **逐项执行并验证**：按任务清单顺序执行，每完成一项在清单中打勾；发现偏差则修正任务清单后再继续

**任务清单格式示例**：

```
## 本次任务清单
- [T1] 需求澄清（识别阻塞项，AI 假设标注）
- [T2] 编写 ATDD 验收场景主表
- [T3] 验收文档交用户确认（闸门）
- [T4] 结构设计
- [T5] TDD 实现（红 → 绿 → 重构）
...
通读手头 **spec、需求、技术说明**；按上方判断标准区分：

| 类型 | 处理方式 |
|------|----------|
| AI 可自行合理推断 | 用 `[假设]` 标注，写入验收文档，不阻塞 |
| 影响实现方向、有多条可能路径的歧义/缺失 | 列入「待确认问题」，阻塞推进 |
...
- **逐项确认 + 自动带出下一项（硬性）**：与用户对话时，**每次只主动推进一项**待确认（默认顺序：**表 A 阻塞项**按 `A-xx` 升序 → **表 B** 按 `B-xx` 升序；若存在依赖，以依赖先决项优先）。用户给出可执行的结论后，代理须**同一轮内**：(1) 把结论写入验收文档（表 A/B、映射表、相关场景 **Then**、必要时「已决议」归档）；(2) **在回复末尾**自动附上**下一项**的、同样**用户视角**的可复制问句（若队列已空，则明确说明「待确认项已清」，并引导进入 **2.1 验收文档交用户确认（闸门）**）。  
...
在 **第 2 节 ATDD 验收场景表**（及 **1.1 待确认问题** 中**阻塞项**已有答复或已标注「接受风险暂不澄清」）成稿后：
...
3. **取得明确结论后再进入第 3 节结构设计**：对方确认无修订，或已根据反馈**改表并再次确认**；将结论记入 **[references/atdd-checklist-template.md](references/atdd-checklist-template.md)** 中的 **「验收文档确认」** 表（时间、结论、是否可开发）。  
4. **禁止**在未经该确认（或用户明示跳过闸门）前，开始 **第 3 节 结构设计** 与后续产码；避免按错误 Then 白写实现。

### 3. 结构设计

**仅在 2.1 验收文档交用户确认（闸门）通过之后**，由验收点反推 **模块边界与接口**；先写清「谁负责什么」再写大段代码；**非必要不扩展**（YAGNI）。
...
在 **2.1 闸门通过后**，将 **第 2 节 ATDD** 中的关键验收路径落成 **E2E**（工具与目录由项目约定）。  
...
- [ ] 若本轮改过主验收正文，是否在对应的 **`.execution-log.md`** 配套文件中**如实追加**了本会话实际对照过的路径（主文仅链接、无冒充、无让用户代填）？  
